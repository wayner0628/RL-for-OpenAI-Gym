...
class Agent():
    ...
    def learn(self):
        ...
        batch_state = torch.Tensor(np.array(observations))
        batch_action = torch.tensor(actions)
        batch_reward = torch.Tensor(rewards)
        batch_next_state = torch.Tensor(np.array(next_observations))
        final_mask = torch.tensor(done) > 0
        batch_action = torch.reshape(batch_action, (32,1))
        state_action_values = self.evaluate_net(batch_state).gather(1, batch_action)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if(np.random.random() < self.epsilon):
                ...
                tmp = self.evaluate_net(torch.Tensor(state)).max(0)[1].detach()
                action = int(tmp)
            # End your code
        return action
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        return self.target_n(torch.Tensor(self.env.reset())).max(0)[0].detach()
        # End your code
