...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        # Begin your code
        state = [
            self.discretize_value(feature, self.bins[i])
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        if done: best_q = 0
        else: best_q = np.amax(self.qtable[tuple(next_state)])
        self.qtable[state[0],state[1],state[2],state[3], action] += self.learning_rate * \
            (reward + self.gamma * best_q - self.qtable[state[0],state[1],state[2],state[3], action])
        # End your code
        # You can add some conditions to decide when to save your table
        global best
        if done:
            if self.check_max_Q() > best:
                best = self.check_max_Q()
    ...
    def check_max_Q(self):
        ...
        return max( self.qtable[state[0],state[1],state[2],state[3],0], \
            self.qtable[state[0],state[1],state[2],state[3],1] )
        # End your code
